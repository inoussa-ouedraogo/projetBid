...
$dest = Join-Path $PSScriptRoot '..\..\bidu\src\Screen\Notofication\Notofication.js'
...
# New implementation for Notification screen (WS via STOMP, unified topic, filtering, pause/clear, persistence)
$content = @'
...
// Build ws(s) URL from API_URL automatically
    ...
    const connect = async () => {
      ...
      const client = new Client({
        ...
        onConnect: () => {
          if (!active) return;
          setConnected(true);
          // Subscribe to unified queue from backend
          client.subscribe('/user/queue/auction-rank', (message) => {
            ...
            try {
              ...
              try {
                Toast.show({
                  type: 'info',
                  text1: newNotif.message,
                  text2: newNotif.detail,
                });
              } catch {}
            } catch (e) {
